...
test('PRF maps to the right schedule months (year 1 → month 1, then year-ends)', () => {
  const map = prfByMonth(TP, N);
  assert.ok(map.has(1)); // year 1 at the beginning
  assert.ok(map.has(12)); // year 2 at end of year 1
  assert.ok(map.has(108)); // year 10 at end of year 9
  assert.ok(!map.has(120)); // nothing at the very end
